...
export default function DebtGraph({ groupId }: { groupId: number }) {
  ...
  const { data: debtGraph, isLoading: graphLoading } = useGetDebtGraph(groupId, { query: { enabled: !!group } });
  ...
  const handleSettle = (edge: typeof debtGraph.edges[0]) => {
    createSettlement.mutate(
      { data: { payerId: edge.fromUserId, receiverId: edge.toUserId, amount: edge.amount, note: "Settled via debt graph" } },
      {
        onSuccess: () => {
