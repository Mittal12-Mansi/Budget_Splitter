        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {groups?.map((group) => {
              ...
              const count = group.memberCount ?? 1;
                  ...
                  <div className="group-card rounded-2xl border bg-card p-5 cursor-pointer flex flex-col h-full">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${
                        ...
                        {myBal > 0 ? <TrendingUp className="h-3 w-3" /> :
                         myBal < 0 ? <TrendingDown className="h-3 w-3" /> :
                         <Minus className="h-3 w-3" />}
                        {myBal > 0 ? "owed" : myBal < 0 ? "owes" : "settled"}
                      </div>
                    </div>
                    ...
                    <div className={`amount text-xl font-bold mb-4 ${
                      myBal > 0 ? "text-emerald-600 dark:text-emerald-400" :
                      myBal < 0 ? "text-rose-600 dark:text-rose-400" :
                      "text-muted-foreground"
                    }`}>
                      {myBal > 0 ? "+" : ""}{myBal.toFixed(2)} {group.currency || "INR"}
                    </div>
